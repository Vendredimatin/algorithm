...
using namespace std;
//很自然的BFS
pair<int,int> op1443[8] = {make_pair(-1,-2),make_pair(-2,-1),make_pair(-2,1),make_pair(-1,2),make_pair(1,-2),make_pair(1,2),make_pair(2,-1),make_pair(2,1)};
int main1443(){
    int n,m,x,y;
    cin >> n >> m >> x >> y;
    ...
    while(!q.empty()){
        ...
        for (int i = 0; i < 8; ++i) {
            int newX = pos.first + op1443[i].first;
            int newY = pos.second + op1443[i].second;
