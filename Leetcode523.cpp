...
https://leetcode.com/problems/continuous-subarray-sum/discuss/679929/C%2B%2B-Code-With-Comments
 比较偏数学的方法，而不是偏编程
 * @param nums
 * @param k
 * @return
...
bool checkSubarraySum(vector<int>& nums, int k) {
    ...
    for (int i = 1; i < n; ++i) {
        for (int j = i-1; j >= 0; --j) {
            int sum = preSum[i] - preSum[j] + nums[j];
